        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {currentNews.map((article) => (
            <Link 
              key={article.id} 
              to={`/news/${article.id}`}
              className="block bg-white/95 p-6 rounded-2xl shadow-inner hover:scale-[1.02] hover:shadow-2xl transition-all border border-transparent hover:border-[#59b8db]"
            >
